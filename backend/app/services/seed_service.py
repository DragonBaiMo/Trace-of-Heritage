...
def run_seed(db: Session) -> None:
    """在应用启动时调用，插入全部演示数据。每张表通过行数判断是否已导入，已有数据则跳过。"""
    admin = db.query(User).filter(User.role == "admin").first()
    user = db.query(User).filter(User.username == "heritage_user").first()
    if not admin:
        logger.warning("管理员用户不存在，跳过种子数据导入")
        return
    if not user:
        logger.warning("heritage_user 不存在，跳过种子数据导入")
        return

    _seed_resources(db, admin)
    _seed_products(db)
    _seed_user_points(db, user)
    _seed_wiki(db)
    _seed_videos(db)
    _seed_digests(db)
    logger.info("种子数据检查/导入完成")


# ---------------------------------------------------------------------------
# 资源
# ---------------------------------------------------------------------------
def _seed_resources(db: Session, admin: User) -> None:
    ...
    for rd in resources_data:
        db.add(Resource(**rd))
    db.commit()
    logger.info("已插入 %d 条资源记录", len(resources_data))
...
def _seed_products(db: Session) -> None:
    ...
    for pd in products_data:
        db.add(Product(**pd))
    db.commit()
    logger.info("已插入 %d 条商品记录", len(products_data))


# ---------------------------------------------------------------------------
# 用户积分
# ---------------------------------------------------------------------------
def _seed_user_points(db: Session, user: User) -> None:
    points_row = db.query(UserPoint).filter(UserPoint.user_id == user.id).first()
    if not points_row:
        db.add(UserPoint(user_id=user.id, balance=520))
        db.commit()
        logger.info("已为 heritage_user 添加 520 积分")
    elif points_row.balance == 0:
        points_row.balance = 520
        db.commit()
        logger.info("已将 heritage_user 积分更新为 520")


# ---------------------------------------------------------------------------
# 百科词条
# ---------------------------------------------------------------------------
def _seed_wiki(db: Session) -> None:
    if db.query(WikiEntry).count() > 0:
        return
    wiki_data = [
        # ===== genre（剧种）=====
        dict(title="京剧", category="genre", status="approved", content="""## 京剧
...
2010年入选联合国教科文组织《人类非物质文化遗产代表作名录》。"""),
        dict(title="昆曲", category="genre", status="approved", content="""## 昆曲
...
2001年入选联合国教科文组织首批《人类口头和非物质文化遗产代表作》。"""),
        dict(title="越剧", category="genre", status="approved", content="""## 越剧
...
《梁山伯与祝英台》《红楼梦》《西厢记》《白蛇传》《祥林嫂》"""),
        dict(title="粤剧", category="genre", status="approved", content="""## 粤剧
...
2009年与粤曲一道入选联合国教科文组织《人类非物质文化遗产代表作名录》。"""),
        dict(title="黄梅戏", category="genre", status="approved", content="""## 黄梅戏
...
严凤英（首创黄梅戏女演员），王少舫，马兰，吴琼"""),
        dict(title="豫剧", category="genre", status="approved", content="""## 豫剧
...
常香玉（"豫剧皇后"）、陈素真、崔兰田、马金凤"""),
        dict(title="秦腔", category="genre", status="approved", content="""## 秦腔
...
### 代表艺术家
李正敏、刘毓中、肖若兰、王天民"""),
        dict(title="川剧", category="genre", status="approved", content="""## 川剧
...
### 代表艺术家
阳友鹤、周慕莲、竞华、陈书舫"""),
        dict(title="评剧", category="genre", status="approved", content="""## 评剧
...
### 代表艺术家
新凤霞（"评剧皇后"）、小白玉霜、赵丽蓉"""),
        dict(title="徽剧", category="genre", status="approved", content="""## 徽剧
...
2006年入选首批国家级非物质文化遗产名录。"""),
        # ===== figure（人物）=====
        dict(title="梅兰芳", category="figure", status="approved", content="""## 梅兰芳（1894—1961）
...
建国后，梅兰芳历任中国京剧院院长、中国文联副主席，致力于京剧教育与传承。1961年病逝，举国同悲。"""),
        dict(title="程砚秋", category="figure", status="approved", content="""## 程砚秋（1904—1958）
...
程砚秋的程派旦角至今仍是京剧各流派中传承最为活跃的流派之一。"""),
        dict(title="荀慧生", category="figure", status="approved", content="""## 荀慧生（1900—1968）
...
荀派在当代依然活跃，北京京剧院等院团有专门的荀派传承梯队。"""),
        dict(title="马连良", category="figure", status="approved", content="""## 马连良（1901—1966）
...
马连良是"四大须生"（马谭杨奚）之首，影响了几代京剧老生演员。"""),
        # ===== term（术语）=====
        dict(title="生旦净末丑", category="term", status="approved", content="""## 生旦净末丑
...
喜剧角色，以白粉涂鼻为标志（小花脸），或扮演奸诈滑稽人物，或扮演机智幽默的小人物，是舞台上的活跃因子。"""),
        dict(title="唱念做打", category="term", status="approved", content="""## 唱念做打
...
与四功相配套的是五法：手（手势）、眼（眼神）、身（身体动作）、法（规矩法度）、步（步法），二者共同构成戏曲表演的完整规范体系。"""),
        dict(title="脸谱", category="term", status="approved", content="""## 脸谱
...
脸谱已成为中国传统文化的代表性视觉符号，广泛应用于工艺品、服装、邮票等领域。"""),
        dict(title="水袖", category="term", status="approved", content="""## 水袖
...
梅兰芳的水袖表演被誉为"梅袖"，成为旦角水袖功夫的最高标准。"""),
    ...
    for wd in wiki_data:
        db.add(WikiEntry(**wd))
    db.commit()
    logger.info("已插入 %d 条百科词条", len(wiki_data))


# ---------------------------------------------------------------------------
# 视频推荐
# ---------------------------------------------------------------------------
def _seed_videos(db: Session) -> None:
    if db.query(VideoRecommendation).count() > 0:
        return
    video_data = [
        dict(title="【京剧经典】梅兰芳《贵妃醉酒》全本", description="梅派代表作，梅兰芳大师饰杨贵妃，水袖舞步精巧绝伦，唱腔甜润，是京剧旦角艺术的巅峰之作。", platform="bilibili", url="https://search.bilibili.com/all?keyword=梅兰芳+贵妃醉酒", thumbnail_url=None, opera_genre="京剧", duration_display="45:12", is_active=True),
        dict(title="【昆曲·青春版】《牡丹亭·游园惊梦》", description="白先勇监制青春版牡丹亭，两岸三地名角联袂，水磨腔婉转悠扬，获誉国际。杜丽娘游园一折尤为精彩。", platform="bilibili", url="https://search.bilibili.com/all?keyword=青春版+牡丹亭+游园惊梦", thumbnail_url=None, opera_genre="昆曲", duration_display="52:30", is_active=True),
        dict(title="【越剧】《梁山伯与祝英台》经典全本", description="越剧最广为人知的剧目，化蝶一折催人泪下。上海越剧院经典版本，唱腔柔美细腻。", platform="bilibili", url="https://search.bilibili.com/all?keyword=越剧+梁山伯与祝英台+全本", thumbnail_url=None, opera_genre="越剧", duration_display="1:38:00", is_active=True),
        dict(title="【粤剧】《帝女花》选段合集", description="粤剧经典中的经典，明末公主长平与周世显的爱情悲歌。香港名家演绎，粤语唱腔典雅婉约。", platform="bilibili", url="https://search.bilibili.com/all?keyword=粤剧+帝女花", thumbnail_url=None, opera_genre="粤剧", duration_display="28:45", is_active=True),
        dict(title="【黄梅戏】严凤英《天仙配·树上鸟儿成双对》", description="黄梅戏宗师严凤英演绎的《天仙配》，朴实清新，旋律优美，是中国最耳熟能详的戏曲唱段之一。", platform="bilibili", url="https://search.bilibili.com/all?keyword=严凤英+天仙配+树上鸟儿", thumbnail_url=None, opera_genre="黄梅戏", duration_display="8:20", is_active=True),
        dict(title="【豫剧】常香玉《花木兰·谁说女子不如男》", description="豫剧皇后常香玉饰演花木兰，高亢激昂的豫剧唱腔令人振奋。这段\"谁说女子不如男\"已成跨时代的文化符号。", platform="bilibili", url="https://search.bilibili.com/all?keyword=常香玉+花木兰+谁说女子不如男", thumbnail_url=None, opera_genre="豫剧", duration_display="6:48", is_active=True),
        dict(title="【川剧】变脸绝技大全——世界级非遗技艺", description="川剧变脸技艺纪录片，从历史渊源到技法揭秘，全面展示这一让全世界惊叹的中国绝活。", platform="bilibili", url="https://search.bilibili.com/all?keyword=川剧+变脸+技艺", thumbnail_url=None, opera_genre="川剧", duration_display="22:15", is_active=True),
        dict(title="【秦腔】《窦娥冤·滚绣球》经典唱段", description="秦腔高亢苍劲的嗓音，演绎窦娥冤的旷世悲愤。西北大地的古老呐喊，震撼人心。", platform="bilibili", url="https://search.bilibili.com/all?keyword=秦腔+窦娥冤+滚绣球", thumbnail_url=None, opera_genre="秦腔", duration_display="12:30", is_active=True),
        dict(title="【评剧】新凤霞《刘巧儿》全剧", description="评剧皇后新凤霞主演，取材真实案件改编，1956年拍摄的经典电影版本，感人至深，是中国戏曲电影的瑰宝。", platform="bilibili", url="https://search.bilibili.com/all?keyword=新凤霞+刘巧儿+评剧", thumbnail_url=None, opera_genre="评剧", duration_display="1:12:00", is_active=True),
        dict(title="【京剧·入门】脸谱艺术与行当介绍", description="专为初学者设计的京剧入门视频，用生动有趣的方式介绍生旦净末丑五大行当与脸谱颜色含义，老少皆宜。", platform="bilibili", url="https://search.bilibili.com/all?keyword=京剧+脸谱+行当+入门", thumbnail_url=None, opera_genre="京剧", duration_display="18:40", is_active=True),
        dict(title="【昆曲】身段·水袖功夫教学示范", description="苏州昆剧院名角示范昆曲基本身段与水袖技法，详细讲解手眼身法步的规范动作，适合爱好者学习。", platform="bilibili", url="https://search.bilibili.com/all?keyword=昆曲+水袖+教学", thumbnail_url=None, opera_genre="昆曲", duration_display="35:00", is_active=True),
        dict(title="【纪录片】中国戏曲·百年传承", description="CCTV出品纪录片，讲述20世纪以来中国戏曲的起伏兴衰、大师风范与新时代传承努力，史料珍贵，感人至深。", platform="bilibili", url="https://search.bilibili.com/all?keyword=中国戏曲+百年传承+纪录片", thumbnail_url=None, opera_genre=None, duration_display="1:20:00", is_active=True),
    ]
    for vd in video_data:
        db.add(VideoRecommendation(**vd))
    db.commit()
    logger.info("已插入 %d 条视频推荐", len(video_data))


# ---------------------------------------------------------------------------
# 每周荐读
# ---------------------------------------------------------------------------
def _seed_digests(db: Session) -> None:
    if db.query(WeeklyDigest).count() > 0:
        return
    digests_data = [
        dict(
    ...
    db.commit()
    logger.info("已插入 %d 期每周荐读", len(digests_data))
